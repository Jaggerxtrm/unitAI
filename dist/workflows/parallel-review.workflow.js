 ...
 */
export async function executeParallelReview(params, onProgress) {
    ...
    const promptBuilder = (backend) => {
        const basePrompt = buildCodeReviewPrompt(files, focus);
...
`;
    ...
    const analysisResult = await logger.timing('parallel-analysis', async () => {
        return await runParallelAnalysis(backendsToUse, promptBuilder, onProgress, (backend) => {
 ...
 */
export const parallelReviewWorkflow = {
...
//# sourceMappingURL=parallel-review.workflow.js.map