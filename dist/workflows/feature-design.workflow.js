 ...
 */
export async function executeFeatureDesign(params, onProgress) {
    const { featureDescription, targetFiles, context, architecturalFocus, implementationApproach, testType, validationBackends = [], attachments = [] } = params;
...
Genera suggerimenti concreti di implementazione (patch outline, rischi, test consigliati).`,
    ...
    // SUMMARY
    // ============================================================================
    const successfulPhases = metadata.phases.filter((p) => p.success).length;
...
${finalOutput}`,
    ...
    return formatWorkflowOutput(`Feature Design: ${featureDescription}`, finalOutput, metadata);
}
/**
 * Definizione del workflow feature-design
 */
export const featureDesignWorkflow = {
    name: 'feature-design',
...
//# sourceMappingURL=feature-design.workflow.js.map